...
FEC_RATE_LIMIT_CALLS = int(os.getenv("FEC_RATE_LIMIT_CALLS", "1000"))  # calls per hour (standard key)
FEC_RATE_LIMIT_PERIOD = int(os.getenv("FEC_RATE_LIMIT_PERIOD", "3600"))  # seconds (1 hour)
FEC_RESULTS_PER_PAGE = int(os.getenv("FEC_RESULTS_PER_PAGE", "100"))  # max allowed by FEC

# Retry configuration
FEC_RETRY_BACKOFF_FACTOR = float(os.getenv("FEC_RETRY_BACKOFF_FACTOR", "2.0"))  # exponential backoff: 1s, 2s, 4s
